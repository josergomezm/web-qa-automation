    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
              ...
              <p class="text-sm text-gray-500 mt-1">
                ...
                <span v-if="suggestedBaseUrl" class="block text-blue-600 text-xs mt-1">
                  ...
                  <button type="button" @click="form.baseUrl = suggestedBaseUrl"
                    class="ml-1 text-blue-700 underline">Use this</button>
                </span>
              </p>
            </div>
            ...
            <div>
              <div class="flex justify-between items-center mb-2">
                ...
                <button type="button" @click="openRecordingModal"
                  ...
                  class="inline-flex items-center px-3 py-1 border border-gray-300 rounded-md bg-white text-xs font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
              ...
              <p v-if="selectedPrerequisites.length > 0" class="text-xs text-amber-600 mb-2">
            ...
            <div>
              ...
              <div class="space-y-2">
                ...
                <button type="button" @click="addFormInput" class="text-blue-600 hover:text-blue-800 text-sm">
                  + Add Form Field
                </button>
              </div>
            </div>
            ...
            <div v-if="recordedSteps.length > 0" class="mt-4 bg-gray-50 p-4 rounded-md border border-gray-200">
              <div class="flex justify-between items-center mb-2">
                <h4 class="text-sm font-medium text-gray-900">Recorded Steps ({{ recordedSteps.length }})</h4>
                <button @click="clearRecording" type="button"
                  class="text-xs text-red-600 hover:text-red-800">Clear</button>
              </div>
              <div class="max-h-40 overflow-y-auto text-xs font-mono bg-white p-2 rounded border border-gray-200">
                <div v-for="(step, index) in recordedSteps" :key="index"
                  class="mb-1 border-b border-gray-100 last:border-0 pb-1 last:pb-0">
                  <span class="font-bold text-blue-600">{{ index + 1 }}. {{ step.action }}</span>
                  <span v-if="step.element" class="text-gray-600"> on {{ step.element }}</span>
                  <span v-if="step.value" class="text-green-600"> value="{{ step.value }}"</span>
                </div>
              </div>
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          <button @click="handleSubmit" :disabled="testStore.loading"
  ...
  if (steps && steps.length > 0) {
    ...
    if (analysis) {
      ...
      if (analysis.credentials) {
        // Initialize credentials object if needed - though it defined in script
        if (!form.value.credentials) {
          form.value.credentials = { username: '', password: '' }
        }
